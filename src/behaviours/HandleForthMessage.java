...
public class HandleForthMessage extends OneShotBehaviour {
	...
	public void action() {
		...
		if (hist.contains(p.not())) {
			...
			r1.setContentObject(new Message(BACK, hist.append(new HistEl(p,
					myAgent, EMPTY_CLAUSE)), EMPTY_CLAUSE));
			ACLMessage r2 = acl_message.createReply();
			r2.setContentObject(new Message(FINAL, hist.append(new HistEl(p,
					myAgent, TRUE_CLAUSE)), TRUE_CLAUSE));
			myAgent.send(r1);
			myAgent.send(r2);
		} else if (myAgent.getKnowledge().contains(p)
				|| hist.contains(new HistEl(p, myAgent, null))) {
			ACLMessage r1 = acl_message.createReply();
			r1.setContentObject(new Message(FINAL, hist.append(new HistEl(p,
					myAgent, TRUE_CLAUSE)), TURE_CLAUSE));
			myAgent.send(r1);
		} else {
			...
			if (LOCAL(SELF).contains(EMPTY_CLAUSE)) {
				...
				r1.setContentObject(new Message(BACK, hist.append(new HistEl(p,
						myAgent, EMPTY_CLAUSE)), EMPTY_CLAUSE));
				ACLMessage r2 = acl_message.createReply();
				r2.setContentObject(new Message(FINAL, hist.append(new HistEl(
						p, myAgent, TRUE_CLAUSE)), TRUE_CLAUSE));
				myAgent.send(r1);
				myAgent.send(r2);
			} else {
				/*
				...
				if (LOCAL.equals(EMPTY_SET)) {
					...
					r1.setContentObject(new Message(BACK, hist
							.append(new HistEl(p, myAgent, TRUE_CLAUSE)),
							TRUE_CLAUSE));
				}
				for (Clause c : LOCAL(SELF))
					for (Literal l : c) {
						...
						for (Agent a : agent.getNeighbours(l)) {
							...
							r1.addReceiver(a);
							r1.setContentObject(new Message(FORTH, hist
									.append(new HistEl(p, myAgent, c)), l));
						}
					}
			}
		}
